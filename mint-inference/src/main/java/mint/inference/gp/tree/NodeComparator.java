...
public class NodeComparator implements Comparator<Chromosome> {
	...
	/**
	 * Returns -1 if fitness of o1 is worse than o2, returns 1 if fitness of o1 is
	 * better, and 0 otherwise.
	 *
	 * Note that "fitness" is a minimisation function, so a lower fitness is
	 * better...
	 *
	 * @param co1
	 * @param co2
	...
	public int compare(Chromosome co1, Chromosome co2) {
		...
		if (fitVal == 0D) {
			...
			if (size == 0) {
				return o2.varsInTree().size() - o1.varsInTree().size();
			} else
